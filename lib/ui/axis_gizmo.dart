...
class AxisGizmo extends StatelessWidget {
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                _angleRow('θ', ctrl.theta.value, Colors.orange),
                _angleRow('φ', ctrl.phi.value,   Colors.cyanAccent),
                _angleRow('r', ctrl.radius.value, Colors.white70, suffix: '%'),
              ],
            ),
          ),
  ...
  Widget _angleRow(String label, double val, Color color, {String suffix = '°'}) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(label, style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'monospace')),
...
class _AxisGizmoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final ax in axes) {
      ...
      _dashLine(canvas, center, negTip,
          ax.color.withOpacity(0.18), 1.2);
      ...
      // Label
      _label(canvas, tip + ax.dir * 10, ax.name,
          ax.color.withOpacity(alpha));
    }
  ...
  void _dashLine(Canvas canvas, Offset from, Offset to,
    ...
    if (len < 1) return;
    final u = d / len;
    double pos = 0; bool drawing = true;
    while (pos < len) {
